...
class EmaStochastic(Signal):
    ...
    def entry_signal(self, df: DataFrame, metadata: dict) -> pd.Series:
        buy_after_knife = (
            (df["end_of_falling_knife"] == True)
            & (df["downtrend_signals"] < 7)
        )
            ...
            #& (df["EMA_5_slope"] > 0)
            #& ((df["prev_close_before_falling_knife"] - df["close"]) / df["prev_close_before_falling_knife"] > 0.015  # Baja > 1.9%
        ...
        buy_after_downtrend = (
            ...
            & (df["downtrend_length"].shift(1) >= 10)
        )

        return (buy_after_knife)  # buy_after_downtrend) # | 
